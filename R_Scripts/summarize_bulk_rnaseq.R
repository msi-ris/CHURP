...
de_list <- paste(out_dir, "DEGs/DE_list.txt", sep = "/")
...
raw_mat <- raw_mat[-which(apply(raw_mat[,seq(-1,-6)],1,var) < 1.5),]
...
# If there is only on grouping in the data, we don't need to run the subsequent tests.
if (n_groups == 1){
write("Only 1 grouping present, skipping differential expression tests.", stderr())
quit(status = 0, save = "no")
} else {

# Estimate common and tag-wise dispersion and generate a design matrix necessary for GLM fitting.
design <- model.matrix(~group, data = edge_mat$samples)
edge_mat <- estimateDisp(edge_mat, design = design)

# Fit the per-feature negative binomial GLM and perform the DE test via quasi-likelihood F-test (edgeR recommended).
fit <- glmQLFit(edge_mat, design)
qlf <- glmQLFTest(fit, coef = 2:n_groups)

# Write out the top DE features to file.
#tags = topTags(qlf, p.value = 0.05) for some reason the p.value cut-off doesn't work at all... so we have to do the somewhat ugly stuff below.
tags = topTags(qlf, n = nrow(qlf$genes))
write.table(tags$table[which(tags$table$FDR < 0.05),], file = de_list, sep = '\t', quote = FALSE, row.names = FALSE)
}
