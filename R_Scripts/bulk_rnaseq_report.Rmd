...
## Experiment Summary
The location of the FASTQ files for this run is ``r fastq_dir``. This run
consists of ``r nsamp`` samples. The path to the reference genome is
...
if(!dir.exists(degdir)) {
    ...
    if(length(deg_files) == 0) {
        ...
        for(i in 1:length(deg_files)) {
            curr_f <- deg_files[i]
            fname <- basename(curr_f)
            gps <- gsub("^DE_", "", curr_f, perl=TRUE)
            gps <- gsub("_list.txt$", "", curr_f, perl=TRUE)
            d <- read.table(curr_f, header=TRUE)
            d_g <- merge(d, gene_names, by.x="genes", by.y="EnsemblID")
            d_g <- d_g[order(d_g$FDR),]
            if(nrow(d_g) < 25) {
