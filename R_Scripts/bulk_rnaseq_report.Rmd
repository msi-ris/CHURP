...
This report was generated with CHURP version ``r params["churp_version"]``.
...
to your computer. You will need them to do further analysis and to upload your 
findings to public repositories such as [GEO](https://www.ncbi.nlm.nih.gov/geo/info/seq.html).
...
## Pipeline Script
We have bundled a version of the pipeline script for you. It is important for 
troubleshooting and reruning your analysis. Click the following link to download 
...
## Samplesheet
We have also bundled a version of the samplesheet. This contains details on the 
methods used and will be needed to rerun the analysis. Click the following link
...
read qualities are shown as they came off the sequencing instrument. Trimmed read qualities are shown if low quality edges of each read were trimmed out with [Trimmomatic](https://github.com/usadellab/Trimmomatic).
...
### Mapping Statistics
The following table summarizes read alignment to the reference genome with the output from
`samtools stats`. 
...
differences in sequencing depth (i.e. number of fragments sequenced in the 
library), like FPKM. It does not correct differences in gene length like FPKM, 
however. You can compare CPM values for the same gene across samples, but 
do not compare CPM values for different genes in the same sample.
...
If the experimental groups and desired comparisons were supplied, and each 
group has at least three replicates, we test for differential expression (DE) 
among them with a quasi-likelihood test (using `glmQLFit` in `edgeR`). 
We apply a `0.05` false discovery rate correction to the DE tests.
If you did not supply the required information or the dataset does not have 
enough replication, then this section will mostly be empty.
...
#### Results Summary
The top differentially expressed genes (DEGs) are given below. Comparisons were 
made based on the DEG Comparisons CSV or XLSX file supplied and reported as 
`Test_Group-Reference_Group`, with `Test_Group` serving as the numerator for 
the fold change calculations. That is, if `logFC` is positive, then the gene 
has higher expression in `Test_Group` than in `Reference_Group`. 
The files in the `DEGs/` directory under the output directory follow this 
naming convention:
...
if(!dir.exists(degdir)) {
    print("No comparisons were defined in the required group spreadsheet, so no DE testing occurred.")
} else {
...
Trimmed reads were re-assessed with FastQC. Length,
...
than the specified minimum length (`--min-len` argument, default 200 bp) were
...
the groups defined by the user. A 0.05 false discovery rate correction
...
script and samplesheet are sufficient to recreate all output files. It will just 
require additional compute time and service units. The following directories 
are of interest:
...
these files if you would like to refer to them in the future. The work directory 
...
sure that the paths to the FASTQ folder, the reference genome alignment index,
and the GTF annotation are correctly specified.
...
For the summary workflow, which includes read counting, differential expression 
testing, and report generation we also use the following programs.
