...
# up multiple genomes, then then shouild hopefully know that and be able to
...
names(sumsheet) <- c(
    ...
    "Trim Options",
    "Strand")
sumsheet[,1:4]            #Show only the name, group and file name
```
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        axis(side=2)
        axis(side=1, at=apply(at, 2, mean), labels=as.character(read_summary$V1[idx]), cex.axis=0.75, las=2)
        cat("\n\n")
    }
} else {
    ...
    legend("top", c("Raw", "Trimmed"), fill=c("#d95f02", "#1b9e77"), ncol=2)
    axis(side=2)
    axis(side=1, at=apply(at, 2, mean), labels=as.character(read_summary$V1), cex.axis=0.75, las=2)
}
```
...
```{r bbduk_plot,eval=F, echo=FALSE, message=FALSE, results="asis"}
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        at <- barplot(
            ...
            axes=FALSE)
        axis(side=1, at=at, labels=as.character(bbduk_summary$SampleName)[idx], las=2, cex.axis=0.75)
    ...
    at <- barplot(
        ...
        main="K-mer Based rRNA Contamination",
    axes=FALSE)
    axis(side=1, at=at, labels=as.character(bbduk_summary$SampleName), las=2, cex.axis=0.75)
    axis(side=2, at=seq(0, 0.6, by=0.05), labels=seq(0, 0.6, by=0.05))
    rect(
        xleft=par("usr")[1],
        xright=par("usr")[2],
...
```{r bbduk_table,eval=F, echo=FALSE, message=FALSE}
### Table
library(knitr)
...
Click on the tab headers to show plots of raw and trimmed read qualities. Raw read qualities are shown as they came off the sequencing instrument.

```{r raw_qual_fun, echo=FALSE, fig.width=8, fig.height=8, results='asis'}
...
plotqual_first <- function(sn, draw_ax, rt) {
    ...
    if(draw_ax) {
        axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(TRUE, F,F,F,F)], labels=as.character(dat$Base)[c(TRUE, FALSE,F,F,F)], las=1, cex.axis=0.75, tck=-0.05)
    }
    ...
    if(!file.exists(qr2_file)) {
        ...
        if(draw_ax) {
            axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(T,F,F,F,F)],
                 labels=as.character(dat$Base)[c(T,F,F,F,F)], las=1, cex.axis=0.75, tck=-0.05)
    }
    }
}
...
plotqual_last <- function(sn,rt) {
    ...
    image(
        ...
        axes=FALSE)
    mtext(side=2, font=2, cex=0.75, las=2, sn)
    axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(T, F, F,F,F)], labels=as.character(dat$Base)[c(T, F,F,F,F)], las=2, cex.axis=0.75, tck=-0.05)
    qr2_file <- paste(params["workdir"], "singlesamples", sn, 
                      paste0(sn, "_2.",rt,"_quals.txt"), sep="/")
    if(!file.exists(qr2_file)) {
        plot(c(0, 1), c(0, 1), ann=F, bty="n", type="n", xaxt="n", yaxt="n")
        ...
        image(
            ...
            col=c("darkred", "indianred", "yellow", "gold", "darkgreen", "green"),
            axes=FALSE)
        axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(TRUE, FALSE,F,F,F)], labels=as.character(dat$Base)[c(TRUE, FALSE,F,F,F)], las=2, cex.axis=0.75, tck=-0.05)
    }
}
...
if(large_dataset) {
    ...
    if(nsamp > 2) {
        par(mar=c(0, 5, 2.5, 0.1))
        for(sample_x in samplenames[2:nsamp-1]){
            dummy <- plotqual(sample_x,rt=read_type)
        }
    }
    ...
    if(large_dataset) {
        ...
        for(i in unique(chunks)) {
            ...
            at <- barplot(
                ...
                ylab="",
                col="#1f78b4")
            axis(side=2, at=at, labels=as.character(unique(rnaseqc_summary$SampleName))[idx], las=2, cex.axis=0.75, lty=0)
            axis(side=1)
            axis(side=3)
            cat("\n\n")
        }
        ...
        at <- barplot(
            ...
            ylab="",
            col="#1f78b4")
        axis(side=2, at=at, labels=as.character(unique(rnaseqc_summary$SampleName)), las=2, cex.axis=0.75, lty=0)
        axis(side=1)
        axis(side=3)
    }
} else {
    ...
    if(large_dataset) {
        ...
        for(i in unique(chunks)) {
            ...
            at <- barplot(
                ...
                ylab="",
                col=bar_cols)
            axis(side=2, at=apply(at, 2, mean), labels=as.character(unique(rnaseqc_summary$SampleName))[idx], las=2, cex.axis=0.75, lty=0)
            axis(side=1)
            axis(side=3)
        ...
        at <- barplot(
            ...
            ylab="",
            col=bar_cols)
        axis(side=2, at=apply(at, 2, mean), labels=as.character(unique(rnaseqc_summary$SampleName)), las=2, cex.axis=0.75, lty=0)
        axis(side=1)
        axis(side=3)
        legend("right", c("Exon", "Intron", "Intergenic"), fill=bar_cols, ncol=1, cex=0.8)
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        at <- barplot(
            ...
            axes=FALSE)
        axis(side=2)
        axis(side=1, at=at, labels=as.character(hisat_summary$SampleName)[idx], cex.axis=0.75, las=2)
        legend(
            "top",
    ...
    at <- barplot(
        ...
        axes=FALSE)
        axis(side=2)
        axis(side=1, at=at, labels=as.character(hisat_summary$SampleName), cex.axis=0.75, las=2)
        legend(
            "top",
            c("Uniquely Mapped", "Multiply Mapped", "Discordantly Mapped", "Unmapped"),
            fill=c("#1f78b4", "#a6cee3", "#b2df8a", "#999999"),
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        at <- barplot(
            ...
            horiz=TRUE)
        axis(side=2, at=apply(at, 2, mean), labels=colnames(subread_sum[-c(1)])[idx], las=2, cex.axis=0.75, lty=0)
        axis(side=1)
  ...
  cat("#### Counts-based Plot \n")
    at <- barplot(
        ...
        main="Subread Summary",
        horiz=TRUE)
    axis(side=2, at=apply(at, 2, mean), labels=colnames(subread_sum[-c(1)]), las=2, cex.axis=0.75, lty=0)
    axis(side=1)
    legend("topright", categories,cex = 0.8, fill=bar_cols, xpd=TRUE)
}
```
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        at <- barplot(
            ...
            main="Subread Summary",
            horiz=TRUE)
        axis(side=2, at=at, labels=colnames(subread_sum[-c(1)])[idx], las=2, cex.axis=0.75, lty=0)
        axis(side=1)
        legend("topright", categories,cex = 0.8, fill=bar_cols, xpd=TRUE)
        cat("\n\n")
    }
  ...
  cat("#### Proportions-based Plot\n")
    at <- barplot(
        ...
        main="Subread Summary",
        horiz=TRUE)
    axis(side=2, at=at, labels=colnames(subread_sum[-c(1)]), las=2, cex.axis=0.75, lty=0)
    axis(side=1)
    legend("topright", categories,cex=0.8, fill=bar_cols, xpd=TRUE)
}
```
