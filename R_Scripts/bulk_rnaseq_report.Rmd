...
plot_dup <- function(sn) {
    r1_dfile <- paste(params["outdir"], "Dup_Stats", paste(sn, "_1_Dups.txt", sep=""), sep="/")
    ...
    r2_dfile <- paste(params["outdir"], "Dup_Stats", paste(sn, "_2_Dups.txt", sep=""), sep="/")
...
at <- barplot(
    ...
    axes=FALSE,
    horiz=TRUE,
    xlim=c(0, max_ct*1.25),
    xlab="Number of Bases in Primary Alignments (Millions)",
    main="Base Positions",
    ylab="",
    col=bar_cols)
axis(side=2, at=apply(at, 2, mean), labels=as.character(picard_rnaseq_summary$SampleName), las=2, cex.axis=0.6, lty=0)
axis(side=1)
legend("right", c("Coding", "UTR", "Intron", "Intergenic"), fill=bar_cols, ncol=1)
```

```{r gdna_plot_prop, echo=FALSE, message=FALSE}
prop_dat <- matrix(
    c(
...
if(all(plot_dat == 0)) {
    plot(c(0, 1), c(0, 1), ann=F, bty="n", type="n", xaxt="n", yaxt="n")
    text(x=0.5, y=0.5, "All strand agreement is 0:\nunstranded lib", cex=2, col="black")
} else {
    at <- barplot(
        plot_dat,
        col="#1f78b4",
