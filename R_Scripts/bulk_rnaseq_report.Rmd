...
```{r raw_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(2.4, 0.6*lg_cutoff), results="asis"}
plotqual_first <- function(sn, draw_ax) {
    allquals <- matrix(0:45)
    image(
        allquals,
...
```{r trim_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(2.4, 0.6*lg_cutoff), results="asis"}
if(length(trim_samp) > 0) {
    plotqual_first <- function(sn, draw_ax) {
        allquals <- matrix(0:45)
        image(
