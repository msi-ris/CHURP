...
```{r subread_summary, echo=FALSE, message=FALSE, fig.width=8, fig.height=1.75*nrow(sheet)}
library(knitr)
...
at <- barplot(
    ...
    horiz=TRUE)
axis(side=2, at=apply(at, 2, mean), labels=colnames(subread_sum[-c(1)]), las=2, cex.axis=0.6)
axis(side=1)
legend("top", categories, fill=bar_cols, cex=0.65, ncol=6, inset=c(0, -0.05), xpd=TRUE)
