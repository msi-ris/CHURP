...
first_samp <- as.character(sheet$V3[1])
fastq_dir <- dirname(first_samp)
```
...
samples <- colnames(hdr)[-c(1)]
t_subread <- as.data.frame(subread_sum[,-c(1)])
rownames(t_subread) <- NULL
colnames(t_subread) <- hdr
t_subread <- cbind("Sample Name"=samples, t_subread)
kable(t_subread, caption="Summary of subread featureCounts assignment.")
```
