...
log_name="${outdir}/Logs/${first_sample}_Analysis.log"
...
log_name="${outdir}/Logs/BulkRNASeq_Summary.log"
...
plot_cov <- function(sn) {
    ...
    cov_dat <- read.table(cf, header=FALSE, stringsAsFactors=FALSE, skip=11)
    plot(
...
This plot will show the breakdown of read assignment by `featureCounts`.

```{r subread_summary, echo=FALSE, message=FALSE}
library(knitr)
...
categories <- c(
    ...
    "Overlapping",
    "Ambiguity")
bar_cols <- c("#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#6a3d9a", "#ffff99")
...
axis(side=1, at=apply(at, 2, mean), labels=colnames(subread_sum[-c(1)]), las=2, cex.axis=0.6)
axis(side=1)
...
kable(subread_sum, caption="Summary of subread featureCounts assignment.")
```
