...
## Read Quality Plots
```{r raw_qual_sum, echo=FALSE, fig.width=8, fig.height=min(0.6*lg_cutoff, 0.6*nsamp), results='asis'}
...
```{r trim_qual_sum, echo=FALSE, fig.width=8, fig.height=min(0.6*lg_cutoff, 0.6*nsamp), results='asis'}
...
if(length(trim_samp) > 0) {
    ...
    if(large_dataset) {
        ...
        dummy <- plotqual_first(trim_samp[1])
            if(nsamp > 2) {
            par(mar=c(1.1, 5, 1.1, 0.1))
            dummy <- sapply(trim_samp[2:nsamp-1], plotqual)
        }
        par(mar=c(2.5, 5, 0.1, 0.1), mgp=c(1.25, 0.15, 0))
...
if(large_dataset) {
    ...
    if(nsamp > 2) {
        par(mar=c(1.1, 5, 1.1, 0.1))
        dummy <- sapply(samplenames[2:nsamp-1], plotqual)
    }
    par(mar=c(2.5, 5, 0.1, 0.1), mgp=c(1.25, 0.15, 0))
    dummy <- plotqual_last(samplenames[nsamp])
}
