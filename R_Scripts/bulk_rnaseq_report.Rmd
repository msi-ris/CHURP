...
```{r raw_read_quals, echo=FALSE, message=FALSE, out.width="750", out.height="750"}
...
plotqual <- function(sn) {
    ...
    dat$X <- 1:nrow(dat)
    # Make a plot
    plot(dat$QMean ~ dat$X, type="n", xlab="Base", ylab="Quality", axes=F, ylim=c(0, 45))
...
```{r trim_read_quals, echo=FALSE, message=FALSE, out.width="750", out.height="750"}
# Use the samplesheet to get the sample names
sheet <- read.table(as.character(params["samplesheet"]), sep="|", header=FALSE, comment.char="#", stringsAsFactors=FALSE)
