...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        at <- barplot(
            ...
            axes=FALSE)
        axis(side=2)
        axis(side=1, at=at, labels=as.character(subdat$SampleName), cex.axis=0.6, las=2)
        legend(
            "top",
...
```{r epe_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*24)}
...
```{r dup_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*24)}
par(mar=c(4, 3.5, 1, 0), mgp=c(2.5, 0.75, 0))
...
```{r gdna_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.8*nrow(sheet), 0.8*24)}
...
```{r rrna_map_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*24)}
par(mar=c(4, 3.5, 1, 0), mgp=c(2.5, 0.75, 0))
...
```{r raw_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.7*nrow(sheet), 0.7*24)}
plotqual_first <- function(sn) {
    allquals <- matrix(0:45)
    image(
        allquals,
...
```{r trim_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.7*nrow(sheet), 0.7*24)}
if(length(trim_samp) > 0) {
    plotqual_first <- function(sn) {
        allquals <- matrix(0:45)
        image(
...
```{r raw_read_quals_fastqc, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(1.75*nrow(sheet), 1.75*24)}
plotqual <- function(sn) {
    qr1_file <- paste(params["workdir"], "singlesamples", sn, paste(sn, "_1.raw_quals.txt", sep=""), sep="/")
    dat <- read.table(qr1_file, header=F, stringsAsFactors=FALSE)
    names(dat) <- c("Base", "QMean", "QMedian", "Q25", "Q75", "Q10", "Q90")
...
```{r subread_summary, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.8*nrow(sheet), 0.8*24)}
library(knitr)
...
```{r subread_summary_prop, echo=FALSE, message=FALSE, fig.height=min(0.4*nrow(sheet), 0.4*24)}
