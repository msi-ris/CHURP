...
if(large_dataset) {
    chunks <- cut(1:nsamp, breaks=unique(c(seq(0, nsamp, by=24), nsamp)), include.lowest=TRUE)
}
```
...
## Read Quality Plots
```{r raw_qual_sum, echo=FALSE, fig.width=8, fig.height=min(0.8*24, 0.8*nrow(sheet)), results='asis'}
...
```{r trim_qual_sum, echo=FALSE, fig.width=8, fig.height=min(0.8*24, 0.8*nrow(sheet)), results='asis'}
...
```{r epe_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*24), results="asis"}
...
```{r dup_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*24), results="asis"}
par(mar=c(4, 3.5, 1, 0), mgp=c(2.5, 0.75, 0))
...
```{r gdna_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.8*nrow(sheet), 0.8*24), results="asis"}
par(mar=c(4, 3.5, 1, 0.5), mgp=c(2.5, 0.75, 0), mfrow=c(1, 2))
gdna_dat <- matrix(
    c(
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        axis(side=1)
        legend("right", c("Exon", "Intron", "Intergenic"), fill=bar_cols, ncol=1)
...
#### Strand Agreement
If the sum of the exon proportion and intron proportion is approximately 50%,
then it is possible that you have have not given the correct strand-specifity
to the pipeline. If you supplied `RF`, try re-running with `FR`, and
*vice versa*. If you have already tried giving the opposite strand-specifity,
or your library is not strand-specific, then it is likely that you have high
levels of genomic DNA contamination in your library.

### rRNA
This plot shows the proportion of alignments that are in annotated rRNA features.

```{r rrna_map_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*24), results="asis"}
par(mar=c(4, 3.5, 1, 0), mgp=c(2.5, 0.75, 0))
...
```{r raw_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.7*nrow(sheet), 0.7*24), results="asis"}
plotqual_first <- function(sn) {
    allquals <- matrix(0:45)
    image(
        allquals,
...
```{r trim_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.7*nrow(sheet), 0.7*24), results="asis"}
if(length(trim_samp) > 0) {
    plotqual_first <- function(sn) {
        allquals <- matrix(0:45)
        image(
...
```{r raw_read_quals_fastqc, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(1.75*nrow(sheet), 1.75*24), results="asis"}
plotqual <- function(sn) {
    qr1_file <- paste(params["workdir"], "singlesamples", sn, paste(sn, "_1.raw_quals.txt", sep=""), sep="/")
    dat <- read.table(qr1_file, header=F, stringsAsFactors=FALSE)
    names(dat) <- c("Base", "QMean", "QMedian", "Q25", "Q75", "Q10", "Q90")
...
```{r trim_read_quals_fastqc, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(1.75*nrow(sheet), 1.75*24), results="asis"}
if(length(trim_samp) > 0) {
    plotqual <- function(sn) {
        qr1_file <- paste(params["workdir"], "singlesamples", sn, paste(sn, "_1.trim_quals.txt", sep=""), sep="/")
        dat <- read.table(qr1_file, header=F, stringsAsFactors=FALSE)
...
```{r subread_summary, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.8*nrow(sheet), 0.8*24), results="asis"}
library(knitr)
...
```{r subread_summary_prop, echo=FALSE, message=FALSE, fig.height=min(0.4*nrow(sheet), 0.4*24), results="asis"}
