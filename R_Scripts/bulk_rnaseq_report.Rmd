...
## Read Quality Plots
```{r raw_qual_sum, echo=FALSE, fig.width=8, fig.height=min(0.6*lg_cutoff, 0.6*nrow(sheet)), results='asis'}
...
```{r trim_qual_sum, echo=FALSE, fig.width=8, fig.height=min(0.6*lg_cutoff, 0.6*nrow(sheet)), results='asis'}
...
```{r epe_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*lg_cutoff), results="asis"}
par(mar=c(4, 3.5, 4, 0.5), mgp=c(2.5, 0.75, 0))
...
```{r dup_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*lg_cutoff), results="asis"}
par(mar=c(4, 3.5, 4, 0.5), mgp=c(2.5, 0.75, 0))
...
```{r gdna_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.6*nrow(sheet), 0.6*lg_cutoff), results="asis"}
par(mar=c(4, 3.5, 4, 0.5), mgp=c(2.5, 0.75, 0), mfrow=c(1, 2))
gdna_dat <- matrix(
    c(
...
```{r rrna_map_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.4*nrow(sheet), 0.4*lg_cutoff), results="asis"}
par(mar=c(4, 3.5, 4, 0.5), mgp=c(2.5, 0.75, 0))
...
```{r raw_read_quals_fastqc, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(2.25*nrow(sheet), 2.25*lg_cutoff), results="asis"}
plotqual <- function(sn) {
    qr1_file <- paste(params["workdir"], "singlesamples", sn, paste(sn, "_1.raw_quals.txt", sep=""), sep="/")
    dat <- read.table(qr1_file, header=F, stringsAsFactors=FALSE)
    names(dat) <- c("Base", "QMean", "QMedian", "Q25", "Q75", "Q10", "Q90")
...
```{r trim_read_quals_fastqc, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(2.25*nrow(sheet), 2.25*lg_cutoff), results="asis"}
if(length(trim_samp) > 0) {
    plotqual <- function(sn) {
        qr1_file <- paste(params["workdir"], "singlesamples", sn, paste(sn, "_1.trim_quals.txt", sep=""), sep="/")
        dat <- read.table(qr1_file, header=F, stringsAsFactors=FALSE)
...
```{r subread_summary, echo=FALSE, message=FALSE, fig.width=8, fig.height=min(0.6*nrow(sheet), 0.6*lg_cutoff), results="asis"}
library(knitr)
...
```{r subread_summary_prop, echo=FALSE, message=FALSE, fig.height=min(0.4*nrow(sheet), 0.4*lg_cutoff), results="asis"}
