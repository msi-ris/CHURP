...
## Read Quality Summary {.tabset}
### Raw Reads

```{r raw_qual_sum, echo=FALSE, message=FALSE, out.width="850", out.height="750"}
...
### Trimmed Reads
```{r trim_qual_sum, echo=FALSE, message=FALSE, out.width="850", out.height="750"}
# Use the samplesheet to get the sample names
sheet <- read.table(as.character(params["samplesheet"]), sep="|", header=FALSE, comment.char="#", stringsAsFactors=FALSE)
...
```{r raw_read_quals, echo=FALSE, message=FALSE, out.width="850", out.height="750"}
# Use the samplesheet to get the sample names
sheet <- read.table(as.character(params["samplesheet"]), sep="|", header=FALSE, comment.char="#", stringsAsFactors=FALSE)
samplenames <- sheet$V1
...
```{r trim_read_quals, echo=FALSE, message=FALSE, out.width="850", out.height="750"}
# Use the samplesheet to get the sample names
sheet <- read.table(as.character(params["samplesheet"]), sep="|", header=FALSE, comment.char="#", stringsAsFactors=FALSE)
samplenames <- sheet$V1[sheet$V7 == "yes"]
if(length(samplenames) > 0) {
...
```{r raw_read_quals_fastqc, echo=FALSE, message=FALSE, out.width="850", out.height="750"}
# Use the samplesheet to get the sample names
sheet <- read.table(as.character(params["samplesheet"]), sep="|", header=FALSE, comment.char="#", stringsAsFactors=FALSE)
samplenames <- sheet$V1
plotqual <- function(sn) {
...
par(mfrow=c(3, 2), mar=c(4, 3.5, 1, 0), mgp=c(2.5, 0.75, 0))
...
```{r trim_read_quals_fastqc, echo=FALSE, message=FALSE, out.width="850", out.height="750"}
# Use the samplesheet to get the sample names
sheet <- read.table(as.character(params["samplesheet"]), sep="|", header=FALSE, comment.char="#", stringsAsFactors=FALSE)
samplenames <- sheet$V1[sheet$V7 == "yes"]
if(length(samplenames) > 0) {
...
kable(hisat_summary, caption="Summary of alignment from HISAT2 based on read counts. Note that if the samples are paired-end, then each fragment is represented by 2 reads.")
```
...
names(hisat_prop) <- c(
    ...
    "Multiply Mapped",
    "Discordantly Mapped")
kable(hisat_prop, caption="Summary of alignment from HISAT2 based on percentage of total reads. Note that if the samples are paired-end, then each fragment is represented by 2 reads.")
```
...
kable(mapping_summary, caption="Summary of read mapping from SAMTools. *:counts both R1 and R2. Divide by 2 to get the number of fragments mapped.")
```
...
shaded region shows the 30th-70th percentile range, and the vertical black lines
show the 10th-90th percentile range.
...
plot(
    ...
    axes=FALSE)
segments(x0=seq_along(is_summary$V1), y1=is_summary$V5, y0=is_summary$V5, lwd=1, col="black")
segments(x0=seq_along(is_summary$V1), y1=is_summary$V6, y0=is_summary$V7, lwd=8, col="grey")
