...
gdna_dat <- matrix(
    c(
        as.numeric(rnaseqc_summary_uns$Value[rnaseqc_summary$Statistic == "Exonic Rate"]),
        as.numeric(rnaseqc_summary_uns$Value[rnaseqc_summary$Statistic == "Intronic Rate"]),
        as.numeric(rnaseqc_summary_uns$Value[rnaseqc_summary$Statistic == "Intergenic Rate"])
        ),
    byrow=TRUE,
    nrow=3
    )
...
at <- barplot(
    ...
    main="Subread Summary",
    horiz=TRUE)
axis(side=2, at=at, labels=colnames(subread_sum[-c(1)]), las=2, cex.axis=0.6)
axis(side=1, at=seq(0, 1, by=0.2), labels=seq(0, 1, by=0.2))
legend("right", categories, fill=bar_cols)
