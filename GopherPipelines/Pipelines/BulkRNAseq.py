...
class BulkRNAseqPipeline(Pipeline.Pipeline):
    ...
    # The programs that we need to report versions for. They should have the
    # same formmat as defined in the __init__.py of the top-level module.
    PROGS = [
        'HISAT2',
        'FASTQC',
        'TRIMMOMATIC',
    ...
    def qsub(self):
        ...
        if os.path.isfile(pname):
            self.pipe_logger.warning('Submission script %s already exists. Overwriting!', pname)
        try:
            handle = open(pname, 'w')
        except OSError:
        ...
        handle.write('# Software versions\n')
        for p in self.PROGS:
            handle.write('# ' + p + ': ' + GopherPipelines.SW_VERS[p] + '\n')
